    ...
    class BCTAI_Chat
    {
        ...
        public function bctai_chatbox_message()
        {
            ...
            if (!wp_verify_nonce($bctai_nonce, 'bctai-chatbox')) {
                ...
                if (!empty($bctai_message)) {
                    ...
                    //20240430 영빈 주석
                    // $bctai_language_file = BCTAI_PLUGIN_DIR . 'admin/chat/languages/' . $bctai_chat_language . '.json';
                    // if (!file_exists($bctai_language_file)) {
                    //     $bctai_language_file = BCTAI_PLUGIN_DIR . 'admin/chat/languages/ko.json';
                    // }
                    // $bctai_language_json = wp_remote_get($bctai_language_file);
                    // //$bctai_languages = json_decode($bctai_language_json, true);
                    // $bctai_languages = 'korean';
                    // $bctai_chat_tone = isset($bctai_languages['tone'][$bctai_chat_tone]) ? $bctai_languages['tone'][$bctai_chat_tone] : 'Professional';
                    // $bctai_chat_proffesion = isset($bctai_languages['proffesion'][$bctai_chat_proffesion]) ? $bctai_languages['proffesion'][$bctai_chat_proffesion] : 'none';


                    // $bctai_greeting_key = 'greeting';

                    // if ($bctai_chat_proffesion != 'none') {
                    //     $bctai_greeting_key .= '_proffesion';
                    // }
                    // $bctai_chat_greeting_message = sprintf($bctai_languages[$bctai_greeting_key], $bctai_chat_tone, $bctai_chat_proffesion . ".\n");
                    // if (!empty($bctai_chat_addition_text)) {
                    //     $site_url = site_url();
                    ...
                    //     $bctai_chat_addition_text = str_replace('[siteurl]', $site_url, $bctai_chat_addition_text);
                    //     $bctai_chat_addition_text = str_replace('[domain]', $domain_name, $bctai_chat_addition_text);
                    //     $bctai_chat_addition_text = str_replace('[sitename]', $sitename, $bctai_chat_addition_text);
                    //     $bctai_chat_addition_text = str_replace('[date]', $date, $bctai_chat_addition_text);
                    // }
                    // if ($bctai_chat_content_aware == 'yes') {
                    //     if ($bctai_chat_with_embedding && !empty($bctai_embedding_content)) {
                    //         $bctai_greeting_key .= '_content';
                    //         $current_context = '"' . $bctai_embedding_content . '"';
                    //         if ($bctai_chat_proffesion != 'none') {
                    //             $bctai_chat_greeting_message = sprintf($bctai_languages[$bctai_greeting_key], $bctai_chat_tone, $bctai_chat_proffesion . ".\n", $current_context);
                    //         } else {
                    //             $bctai_chat_greeting_message = sprintf($bctai_languages[$bctai_greeting_key], $bctai_chat_tone . ".\n", $current_context);
                    //         }
                    //         if ($bctai_chat_addition && !empty($bctai_chat_addition_text)) {
                    //             $bctai_chat_greeting_message .= ' ' . sprintf($bctai_languages[$bctai_greeting_key . '_extra'], $bctai_chat_addition_text);
                    ...
                    //             }
                    //         }
                    //     } elseif ($bctai_chat_addition && !empty($bctai_chat_addition_text)) {
                    //         $bctai_greeting_key .= '_content';
                    //         $bctai_chat_greeting_message .= ' ' . sprintf($bctai_languages[$bctai_greeting_key . '_extra'], $bctai_chat_addition_text);
                    //     }
                    // } elseif ($bctai_chat_addition && !empty($bctai_chat_addition_text)) {
                    ...
                    //     $bctai_chat_greeting_message .= ' ' . sprintf($bctai_languages[$bctai_greeting_key . '_extra'], $bctai_chat_addition_text);
                    // }
                    // if (!empty($bctai_user_name)) {
                    //     $bctai_chat_greeting_message .= '. ' . $bctai_user_name;
                    // }
                    // $bctai_result['greeting_message'] = $bctai_chat_greeting_message;
                    /////////////////////////////////////////////////////////////////////////////////
                    ...
                    if ($bctai_ai_model === 'gpt-3.5-turbo' || $bctai_ai_model === 'gpt-3.5-turbo-16k' || $bctai_ai_model == 'gpt-4' || $bctai_ai_model == 'gpt-4-32k') {
                        $bctai_data_request = [
                            'model' => $bctai_ai_model,
                            // 'model' => 'ft:gpt-3.5-turbo-1106:bctone::8UW5nczP',
                        ...
                        $bctai_data_request = [
                            ...
                            'presence_penalty' => floatval($bctai_chat_presence_penalty),
                            'top_p' => floatval($bctai_chat_top_p),
                            //'best_of' => intval($bctai_chat_best_of)
                        ];
                        $complete = $open_ai->chat($bctai_data_request);
        ...
        public function wpaicg_embeddings_result_qdrant($bctai_chat_provider, $open_ai, $bctai_qdrant_api, $bctai_qdrant_endpoint, $default_qdrant_collection, $bctai_message, $wpaicg_chat_embedding_top, $namespace = false)
        {
            ...
            if(!empty($bctai_qdrant_api) && !empty($bctai_qdrant_endpoint && !empty($default_qdrant_collection))) {
                ...
                //         try {
                //             $embedding_engine = WPAICG_Util::get_instance()->initialize_embedding_engine($selected_embedding_provider, $bctai_chat_provider);
                //         } catch (\Exception $e) {
                ...
                //             $model = $model_parts[1];
                //             try {
                //                 $embedding_engine = WPAICG_Util::get_instance()->initialize_embedding_engine($model_parts[0], $bctai_chat_provider);
                //             } catch (\Exception $e) {
                //                 $result['msg'] = $e->getMessage();
                //                 return $result;
                //             }
