...
?>
<h1 style="font-weight: bolder; margin-bottom:20px;"><?php echo __('Datasets', 'bctai') ?></h1></br>

<h1 class="wp-heading-inline"><?php echo esc_html__('Files','bctai')?></h1>
<!-- <h1 style="font-weight: bolder;"><?php echo __('Files', 'bctai') ?></h1> -->
<button href="javascript:void(0)" class="page-title-action bctai_sync_files" style="color: #8040ad; border: 1px solid #8040ad;"><?php echo esc_html__('Sync Files','bctai')?></button>
<table class="wp-list-table widefat fixed striped table-view-list comments">
    <thead>
        <tr>
            <th><?php echo esc_html__('ID','bctai')?></th>
            <th style="width: 50px;"><?php echo esc_html__('Size','bctai')?></th>
    ...
    if($bctai_files && is_array($bctai_files) && count($bctai_files)):
        foreach($bctai_files as $bctai_file):
            ...
            <tr>
                ...
                <td>
                    <button data-id="<?php echo esc_html($bctai_file->ID);?>" class="button button-small bctai_create_fine_tune" style="color: #800ad;border-color: #8040ad;"><?php echo esc_html__('Create Fine-Tune','bctai')?></button>
                    <button data-id="<?php echo esc_html($bctai_file->ID);?>" class="button button-small bctai_retrieve_content" style="color: #8040ad;border-color: #8040ad;"><?php echo esc_html__('Retrieve Content','bctai')?></button>
                    <button data-id="<?php echo esc_html($bctai_file->ID);?>" class="button button-small button-link-delete bctai_delete_file" style="border-color: #d63638;"><?php echo esc_html__('Delete','bctai')?></button>
                </td>
            </tr>
        <?php
        endforeach;
