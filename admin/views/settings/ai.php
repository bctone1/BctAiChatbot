...
<h2 class="sectionTitle">LLM Providers</h2>
...
<div class="Google_wrap" style=" display:<?php echo $bctai_chat_provider =='Google' ? 'block':'none'?>">
    ...
    <div class="columnWrap">
        <div class="columnWrap column2">
            <div class="formTitle"><strong>Api Key</strong></div>
            <div class="formContent">
                <input type="text" id="wpaicg_google_api_key" name="wpaicg_google_api_key"value="<?php echo esc_html($wpaicg_google_api_key); ?>">
                <div class="link underline"><a href="https://openrouter.ai/keys" target="_blank" style="color: #F53706;">Get Your Key</a></div>
            </div>
        </div>
    </div>
...
<div class="section" style="padding-top:60px;">
    <div class="sectionHeader">
        <h2 class="sectionTitle">Vector Storage Setting</h2>
    </div>
        ...
        <div class="columnWrap">
            <div class="columnWrap column2">
                ...
                <div class="formContent">
                    ...
                    <div class="link underline"><a href="https://pinecone.io/" target="_blank" style="color: #F53706;">Get Your Key</a></div>
                </div>
            </div>
        </div>
...
<h2 class="sectionTitle">Embedding Model Providers</h2>

<?php 
$Embedding_Provider = get_option('Embedding_Provider','OpenAI');
$OpenAI_Embedding_model = get_option('OpenAI_Embedding_model','text-embedding-3-small');
$Google_Embedding_model = get_option('Google_Embedding_model','text-embedding-004');
    ...
    <div class="columnWrap column2">
        ...
        <div class="formContent">
            <select id="Embedding_Provider" name="Embedding_Provider">
                <option value="OpenAI"><?php echo __('OpenAI', 'bctai') ?></option>
                <option <?php echo $Embedding_Provider == 'Google' ? ' selected': ''?> value="Google"><?php echo __('Google', 'bctai') ?></option>
            </select>
        </div>
    </div>
</div>
...
<div class="columnWrap" id="OpenAI_embedding_wrap" style=" display:<?php echo $Embedding_Provider =='OpenAI' ? 'block':'none'?>">
    <div class="columnWrap column2">
        <div class="formTitle"><strong>Embedding Model</strong></div>
        <div class="formContent">
            <select name="OpenAI_Embedding_model">
                <option <?php echo $OpenAI_Embedding_model == 'text-embedding-3-small' ? ' selected': ''?> value="text-embedding-3-small">text-embedding-3-small (1,536)</option>
                <option <?php echo $OpenAI_Embedding_model == 'text-embedding-3-large' ? ' selected': ''?> value="text-embedding-3-large">text-embedding-3-large (3,072)</option>
                <option <?php echo $OpenAI_Embedding_model == 'text-embedding-ada-002' ? ' selected': ''?> value="text-embedding-ada-002">text-embedding-ada-002 (1,536)</option>
            </select>
        </div>
    </div>
</div>

<div class="columnWrap" id="Google_embedding_wrap" style=" display:<?php echo $Embedding_Provider =='Google' ? 'block':'none'?>">
    <div class="columnWrap column2">
        <div class="formTitle"><strong>Embedding Model</strong></div>
        <div class="formContent">
            <select name="Google_Embedding_model">
                <option <?php echo $Google_Embedding_model == 'text-embedding-004' ? ' selected': ''?> value="text-embedding-004">text-embedding-004 (768)</option>
                <option <?php echo $Google_Embedding_model == 'embedding-001' ? ' selected': ''?> value="embedding-001">embedding-001 (768)</option>
            </select>
        </div>
    </div>
</div>
...
<p><?php echo __('You can create indexes using the index builder.', 'bctai') ?></p>
<p><?php echo __('Index Builder lets you monitor and automatically update your content.', 'bctai') ?></p>
    ...
    <div class="columnWrap column2">
        ...
        <div class="formContent">
            <div class="inputButtonArea">
                ...
                <button type="button" class="btn btnL bgPrimary bctai_sync_google_voices"><span><?php echo esc_html__('SYNC','')?></span></button>
            </div>
...
<script>
    jQuery(document).ready(function ($) {
        ...
        $('#Embedding_Provider').change(function(){
            if(this.value == 'OpenAI'){
                $('#OpenAI_embedding_wrap').css('display','block');
                $('#Google_embedding_wrap').css('display','none');
            }else if(this.value == 'Google'){
                $('#OpenAI_embedding_wrap').css('display','none');
                $('#Google_embedding_wrap').css('display','block');
            }
        });
